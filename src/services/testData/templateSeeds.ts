...
export function buildTemplateRecords(): FormRecord[] {
  ...
  make('decision_log', `决策 - ${daysAgo(15)}`, {
    ...
    time_pressure: '中等',
    idea_experience: [
      { idea: '切换到团队更熟悉的新框架', target_cause: '根因' },
    ],
    idea_observation: [
      { idea: '同类项目社区活跃度与招聘热度上升，长期看生态更健康', target_cause: '根因' },
      { idea: '短期先控制维护成本（减少改动频率）', target_cause: '表因' },
    ],
    idea_innovation_scamper: [
      { scamper: 'S 替代', guide: '能否换人 / 换物 / 换流程 / 换渠道？', solution: '替换为核心模块优先迁移的新方案', target_cause: '根因' },
      { scamper: 'C 组合', guide: '能否与其他方案、资源或环节合并？', solution: '', target_cause: '' },
      { scamper: 'A 调整', guide: '能否借鉴其他领域 / 行业的现成做法？', solution: '', target_cause: '' },
      { scamper: 'M 修改', guide: '能否改变形态、规模、参数或时间安排？', solution: '不一定要全量切换，先改核心模块', target_cause: '两者兼治' },
      { scamper: 'P 他用', guide: '能否换个用途、换个使用场景？', solution: '', target_cause: '' },
      { scamper: 'E 消除', guide: '能否去掉某些部分或环节？', solution: '', target_cause: '' },
      { scamper: 'R 重排', guide: '能否颠倒顺序、角色或因果关系？', solution: '其余模块渐进迁移，重排上线顺序', target_cause: '根因' },
    ],
    options_analysis: [
