...
export const decisionLogTemplate: FormTemplate = {
  ...
  sections: [
    ...
    },
    {
      ...
      title: '问题与根因',
      description: '明确要解决的问题，区分表因与根因（决策方案必须针对根因，否则治标不治本）——问题/表因/根因由根因分析工具通过 JSON 导入，无需手动填写',
      fields: [
        {
          id: 'problem_cause_import',
          label: '从根因分析工具导入',
          type: 'jsonImport',
          priority: 'required',
          placeholder: '粘贴根因分析工具复制的 JSON，例如：{"problemDescription": "…", "rootCauses": ["…"], "surfaceCauses": ["…"]}',
          hint: '在根因分析工具（https://promatheus-ltsc.github.io/root-cause-analysis/）完成 5Why 分析后复制 JSON，粘贴到这里点击「解析并导入」，自动填入下方三个字段',
        },
        { id: 'problem_statement', label: '已明确的问题', type: 'textarea', required: true, readOnly: true, placeholder: '由 JSON 导入（problemDescription）', priority: 'required', hint: '来自根因分析工具的问题描述，只读' },
        { id: 'problem_surface_cause', label: '问题的表因（表面原因）', type: 'textarea', required: true, readOnly: true, placeholder: '由 JSON 导入（surfaceCauses）', priority: 'required', hint: '表因 = 问题的表面/直接原因（来自根因分析工具 surfaceCauses），只读' },
        { id: 'problem_root_cause', label: '问题的根因（根本原因）', type: 'textarea', required: true, readOnly: true, placeholder: '由 JSON 导入（rootCauses）', priority: 'required', hint: '根因 = 通过 5Why 追问得到的根本原因（来自根因分析工具 rootCauses），只读' },
    ...
    },
    {
      ...
      fields: [
        {
          id: 'idea_experience',
          label: '选项生成 · 经验维度（立即反应）',
          type: 'table',
          priority: 'recommended',
          hint: '逐条写下凭直觉/过往经验想到的方案，并在「针对」列标注它主要解决表因还是根因——优先针对根因',
          tableColumns: [
            { id: 'idea', label: '经验想法', type: 'text', width: '78%' },
            { id: 'target_cause', label: '针对', type: 'select', options: ['表因', '根因', '两者兼治'], width: '22%' },
          ],
        },
        {
          id: 'idea_observation',
          label: '选项生成 · 观察/归纳/推理维度',
          type: 'table',
          priority: 'recommended',
          hint: '基于观察与推理逐条写下方案，并标注针对表因还是根因',
          tableColumns: [
            { id: 'idea', label: '推理想法', type: 'text', width: '78%' },
            { id: 'target_cause', label: '针对', type: 'select', options: ['表因', '根因', '两者兼治'], width: '22%' },
          ],
        },
        {
          ...
          type: 'table',
          priority: 'recommended',
          hint: '按 SCAMPER 七个角度逐项提问，每个角度都试着提出一个创意解决方案，并标注针对表因还是根因',
          tableColumns: [
            { id: 'scamper', label: '法则', type: 'text', width: '12%' },
            { id: 'guide', label: '引导问题', type: 'text', width: '34%' },
            { id: 'solution', label: '创意方案', type: 'text', width: '38%' },
            { id: 'target_cause', label: '针对', type: 'select', options: ['表因', '根因', '两者兼治'], width: '16%' },
          ],
          defaultValue: [
            { scamper: 'S 替代', guide: '能否换人 / 换物 / 换流程 / 换渠道？', solution: '', target_cause: '' },
            { scamper: 'C 组合', guide: '能否与其他方案、资源或环节合并？', solution: '', target_cause: '' },
            { scamper: 'A 调整', guide: '能否借鉴其他领域 / 行业的现成做法？', solution: '', target_cause: '' },
            { scamper: 'M 修改', guide: '能否改变形态、规模、参数或时间安排？', solution: '', target_cause: '' },
            { scamper: 'P 他用', guide: '能否换个用途、换个使用场景？', solution: '', target_cause: '' },
            { scamper: 'E 消除', guide: '能否去掉某些部分或环节？', solution: '', target_cause: '' },
            { scamper: 'R 重排', guide: '能否颠倒顺序、角色或因果关系？', solution: '', target_cause: '' },
          ],
        },
