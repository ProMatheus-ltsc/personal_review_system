...
- **投资检查清单 · 代码中心三角色模型**：
  ...
  - 买入/卖出决策分别按**可配置等待期**（默认 30 天）解锁复盘
  - 仓位单支持持有中复盘（表格形式）与清仓后投资周期复盘（等待期可配置）
  - 决策日志的**长期复盘等待期**同样可配置（默认 30 天）
  - 结构化四层数据模型（Position + Trade + Trade Review + Position Review）
- **测试账户**：内置独立测试账户（`admin` / `admin`），各场景复盘等待期默认 0 天（机制与普通账户一致、页面可配置），自动填充覆盖各场景的测试数据，便于验收测试且不影响其他账户
...
| 复盘等待期 | **默认 0 天**（买入/卖出/投资周期/决策日志长期复盘），机制与普通账户完全一致、页面可配置，便于测试 |
...
| `BABA` | 刚清仓 5 天 | 验证时间锁（admin 默认等待期 0 → 立即可复盘） |
...
│   ├── PhaseIndicator.tsx      # 多阶段进度指示器（显示等待期倒计时）
...
├── utils/                      # 工具函数
│   ├── formValidation.ts           # 表单验证 + 阶段计算（含等待期时间锁）
...
5. **投资检查清单**：
   ...
   - 买入单/卖出单填写决策后，按「复盘等待期」解锁对应复盘（默认 30 天，可在**数据管理页**按场景配置：买入/卖出/投资周期/决策日志长期复盘各自独立）
   - 卖出复盘为**单次表单**（非可重复段），自动关联本笔卖出，无需手动选择卖出交易
   - 仓位单汇总同代码所有买卖明细（表格形式），清仓后按等待期设置解锁投资周期复盘
   - 复盘 tab 在等待期内显示 🔒 锁页面，点击提示剩余解锁天数
...
12. **多阶段模板**：投资检查清单支持"买入 → 持有 → 卖出 → 复盘"多阶段；各场景复盘等待期均可配置（admin 默认 0 天便于测试，普通账户默认 30 天）
